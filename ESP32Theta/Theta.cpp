...
String Theta::SimpleHttpProtocol(const String sPostGet, String sUrl, String sHost, int iPort, String strData, unsigned int uiTimeoutSec) {
	unsigned long ulStartMs;
	unsigned long ulCurMs;
	unsigned long ulElapsedMs;
	unsigned long ulTimeoutMs;
	int iDelimiter = 0;
	unsigned int iCheckLen = 0;
	int iResponse = 0;
...
//Use WiFiClient class to create TCP connections
	if (!client.connect(sHost.c_str(), iHttpPort)) {
...
//Send Request
	client.print(
			String(sPostGet) + " " + sUrl + " HTTP/1.1\r\n" + "Host: " + String(sHost) + ":" + String(iHttpPort) + "\r\n"
					+ "Content-Type: application/json;charset=utf-8\r\n" + "Accept: application/json\r\n" + "Content-Length: "
					+ String(strData.length()) + "\r\n" + "Connection: close\r\n" + "\r\n");
	if (strData.length() != 0) {
...
String Theta::getInfo(void) {
	...
	String strJson = SimpleHttpProtocol("GET", strUrlInfo, strHost, iHttpPort, strSendData, HTTP_TIMEOUT_NORMAL);
...
int Theta::postGetState(void) {
	...
	String strSendData = String("");
	String strJson = SimpleHttpProtocol("POST", strUrlState, strHost, iHttpPort, strSendData, HTTP_TIMEOUT_STATE);
...
int Theta::postStartSession(void) {
	...
	String strJson = SimpleHttpProtocol("POST", strUrlCmdExe, strHost, iHttpPort, strSendData, HTTP_TIMEOUT_NORMAL);
...
int Theta::postStartCapture(void) {
	...
	String strJson = SimpleHttpProtocol("POST", strUrlCmdExe, strHost, iHttpPort, strSendData, HTTP_TIMEOUT_STARTCAP);
//Serial.println("JSON:[" + strJson + "], len=" + strJson.length() );
...
int Theta::postStopCapture(void) {
	...
	String strJson = SimpleHttpProtocol("POST", strUrlCmdExe, strHost, iHttpPort, strSendData, HTTP_TIMEOUT_STOPCAP);
//Serial.println("JSON:[" + strJson + "], len=" + strJson.length() );
...
int Theta::postCommandsStatus(void) {
	...
	String strJson = SimpleHttpProtocol("POST", strUrlCmdStat, strHost, iHttpPort, strSendData, HTTP_TIMEOUT_CMDSTAT);
//Serial.println("JSON:[" + strJson + "], len=" + strJson.length() );
